...
export function SkillAnalysisOverlay({ candidate, position = "right" }: SkillAnalysisOverlayProps) {
  ...
  const progressionData = useMemo(() => {
    ...
    const top3   = topSkills.slice(0, 3);                      // 상위 3개 스킬 표시
    const pool   = Object.keys(levelIndex);
  ...
  return (
    ...
    {!loading && error && <div className="text-xs text-red-500">에러: {error}</div>}
      ...
      >
        <div className="grid grid-cols-2 gap-8">
          ...
          <div className="space-y-6">
            ...
            {/* Line Chart */}
            <div>
              <h4 className="font-medium mb-3">지원자 기술 숙련도 추이</h4>
              <div className="h-48">
                <ResponsiveContainer width="100%" height="100%">
                  <LineChart data={progressionData} margin={{ top: 20, right: 30, left: 20, bottom: 5 }}>
                    ...
                    <YAxis domain={[70, 100]} />
                    <Tooltip />
                    <Line type="monotone" dataKey={topSkills[0]} stroke="#FFA500" strokeWidth={2} dot={{ r: 4 }} />
                    <Line type="monotone" dataKey={topSkills[1]} stroke="#FF8C42" strokeWidth={2} dot={{ r: 4 }} />
                    <Line type="monotone" dataKey={topSkills[2]} stroke="#20B2AA" strokeWidth={2} dot={{ r: 4 }} />
                  </LineChart>
                </ResponsiveContainer>
              </div>
            </div>
